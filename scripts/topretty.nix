...
let
  ...
  getSortPriority = name: value:
    if !isAttrs value || !(value ? url) then 3 
    else if value ? url && length (attrNames value) == 1 then 1
    else if hasPrefix "git+file" value.url then 4
    else 2;
