{
  ...
  options.modules.themes = let
    themeNames = ["catppuccin" "min" "solarized" "vesper" "carbon" "gruvbox"];
  in
  ...
  options.modules.desktop.colorscheme = {
    ...
    theme = lib.mkOption {
      type = lib.types.enum ["gruvbox" "min" "solarized" "vesper" "carbon" "catppuccin"];
      default = "vesper";
  ...
  in
    lib.mkMerge [
      (lib.mkMerge (
        ...
        ["catppuccin" "min" "solarized" "vesper" "carbon" "gruvbox"]
      ))
