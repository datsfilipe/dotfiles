  ...
  Widget.Button({
    onClicked: () => {
      ...
      Hyprland.sendMessage(`dispatch exec ${app.executable}`)
        .then((e) => print(e)) // eslint-disable-line
    ...
    setup: (button) => {
      button.app = app
      button.connect('destroy', () => print('BUTTON DESTROY', app.name)) // eslint-disable-line
    },
...
const SearchBox = () => {
  const fzf = new Fzf(Applications.list.map(AppButton), {
    ...
    tiebreakers: [(a, b, _) => b.item.app._frequency - a.item.app._frequency], // eslint-disable-line
  })
